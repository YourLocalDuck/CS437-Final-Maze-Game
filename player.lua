...
function Player:new()
    ...
    map.world:add(self, self.x, self.y, self.width * self.imageScale * .8, self.height * self.imageScale)
end
...
function Player:draw()
    if (self.currentState == "idle") then
        if (facing == 0) then
            love.graphics.draw(
            self.Idleimage,
            self.idleFrames[math.floor(currentFrame%self.animationFrames[1]+1)],
            self.x,
            self.y,
            0,
            self.imageScale,
        ...
        if (facing == 0) then
            love.graphics.draw(
            self.movingImage,
            self.movingFrames[math.floor(currentFrame%self.animationFrames[2]+1)],
            self.x,
            self.y,
            0,
            self.imageScale,

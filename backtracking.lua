...
grid2 = {}
for i = 0, height do grid2[i] = {} end

function DX(dir)
    if (dir == E) then return 1 end
    if (dir == W) then return -1 end
    if (dir == N) then return 0 end
    if (dir == S) then return 0 end
end

function DY(dir)
    if (dir == E) then return 0 end
    if (dir == W) then return 0 end
    if (dir == N) then return 1 end
    if (dir == S) then return -1 end
end
...
function between(test, num1, num2)
    if (test >= num1 and test <= num2) then return true end
    return false
end
...
function CarvePassagesHandler()
    ...
    for i = 0, #grid2 - 1 do
        for j = 0, #grid2[i] - 1 do print(grid2[j][i]) end
        print("line" .. i)
    end
    return grid2
end
...
function CarvePassages(cx, cy, grid)
    ...
    for i = 1, #directions do
        ...
        if between(ny, 0, height - 1) and between(nx, 0, width - 1) and
            grid[ny][nx] == nil then
